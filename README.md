...
本系统实现了可视化工作流编排功能，支持创建、编辑和运行复杂的AI工作流：

### 核心功能

* **可视化节点编排**：基于ReactFlow实现直观的拖拽式节点编排
* **多种节点类型**：支持LLM、工具调用、条件分支、知识库等多种节点类型
* **节点连接与编辑**：支持节点之间的连线、属性编辑和删除
* **实时预览**：支持工作流实时预览和测试运行
* **保存与管理**：支持工作流的保存、列表查看和详情编辑

### 节点类型

* **LLM节点**：配置大语言模型调用，设置提示词和参数
* **工具节点**：配置API调用、代码执行或网络搜索等工具
* **条件节点**：设置分支条件，实现复杂的判断逻辑
* **输入节点**：接收用户输入，作为工作流起点
* **输出节点**：处理工作流结果，返回最终输出
* **知识库节点**：配置RAG知识检索，增强回复质量

### 工作流界面

* **节点选择器**：提供多种节点类型供用户选择添加
* **属性编辑面板**：编辑节点属性、参数和配置
* **画布操作**：支持缩放、拖拽、对齐等画布操作
* **右键菜单**：提供节点和画布的快捷操作菜单
* **工具栏**：提供预览、保存、发布等操作按钮

### 技术实现

* 基于ReactFlow构建的响应式画布系统
* 自定义节点和连接线组件，支持不同类型节点的特殊渲染
* 使用TypeScript强类型定义，确保类型安全
* 实现工作流状态管理，支持撤销、重做等操作
* 支持工作流的导入导出功能
