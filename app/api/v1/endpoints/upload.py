...
async def upload_files(
    ...
    if not files: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="没有提供文件。")
    temp_file_paths = []; original_filenames = []; task_id = None
    try:
        try: os.makedirs(settings.upload_temp_dir, exist_ok=True)
        except OSError as e: logger.error(f"无法创建或访问临时上传目录: {settings.upload_temp_dir} - {e}"); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="服务器无法存储上传文件。")
        for file in files:
            if not file.filename: logger.warning("收到一个没有文件名的上传文件，已跳过。"); continue
            allowed_extensions = {".pdf", ".docx", ".md"}; _, ext = os.path.splitext(file.filename)
            if ext.lower() not in allowed_extensions: logger.warning(f"收到不允许的文件类型 '{ext}' (来自文件 '{file.filename}')，已跳过。"); continue
            ...
            try:
                ...
                temp_file_paths.append(temp_file_path); logger.debug(f"文件 '{file.filename}' 已保存到 '{temp_file_path}'")
            except Exception as e: logger.error(f"保存文件 '{file.filename}' 到 '{temp_file_path}' 时出错: {e}"); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"无法保存文件 {file.filename}: {e}")
            finally: await file.close()
        if not temp_file_paths: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="未提供有效或允许的文件类型。允许的类型: PDF, DOCX, MD。")
        ...
        try:
            task = process_document_batch.apply_async(args=[temp_file_paths, original_filenames, target_collection], queue='document_processing')
            task_id = task.id
            logger.info(f"文档处理任务已成功入队，Task ID: {task_id}", extra={"task_id": task_id, "filenames": original_filenames, "collection": target_collection})
             ...
             for path in temp_file_paths: # Cleanup on failure to queue
                 if os.path.exists(path): try: os.remove(path); except OSError: logger.error(f"无法清理临时文件 {path} 在 Celery 发送失败后。")
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"无法安排文件处理任务: {e}")
        return AsyncTaskResponse(message="文件已接收并正在后台处理。", task_id=task_id, filenames=original_filenames, collection_name=target_collection)
    except HTTPException as http_exc:
         for path in temp_file_paths: # Cleanup on known HTTP errors during processing
              if os.path.exists(path): try: os.remove(path); except OSError: pass
         raise http_exc
    except Exception as e:
        logger.exception(f"处理上传请求时发生意外错误: {e}")
        for path in temp_file_paths: # Cleanup on general errors
             if os.path.exists(path): try: os.remove(path); except OSError: pass
